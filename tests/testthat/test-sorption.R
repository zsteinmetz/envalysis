test_that("sorption() handles input errors correctly", {
  sorption(conc = 1:5, c(K = 4)) |> expect_error()
  sorption(conc = 1:5, c(Kd = 2.5), type = "xy") |> expect_error()
})
...
test_that("sorption() output is correct", {
  ...
  sorption(conc = 1:5, c(K = 4, n = 0.6), type = "freundlich") |> round(2) |> 
    expect_equal(c(4.00, 6.06, 7.73, 9.19, 10.51))
  
  sorption(conc = 1:5, c(KL = 2, qmax = 10), type = "langmuir") |> round(2) |> 
  ...
  sorption(conc = 1:5, c(A = 30, B = 0.8), type = "redlich") |> round(2) |> 
